...
static void freeproce(struct proc *p);
...
freethread(&p->threads[i]); // Free all threads associated withthe process
}
}
...
p->current_thread = 0; 
//Initialize current_thread to indicate no active thread
}
}
